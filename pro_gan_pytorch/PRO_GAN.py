...
class Generator(th.nn.Module):
    ...
    def __init__(self, depth=7, latent_size=512, use_eql=True):
        ...
        if self.use_eql:
            from pro_gan_pytorch.CustomLayers import _equalized_conv1d
            self.toRGB = lambda in_channels: \
                _equalized_conv1d(in_channels, 1, 1, bias=True)
        else:
            from torch.nn import Conv1d
            self.toRGB = lambda in_channels: Conv1d(in_channels, 1, 1, bias=True)
    ...
    def __init__(self, height=7, feature_size=512, use_eql=True):
        ...
        if self.use_eql:
            ...
            self.fromRGB = lambda out_channels: \
                _equalized_conv1d(1, out_channels, 1, bias=True)
        else:
            from torch.nn import Conv1d
            self.fromRGB = lambda out_channels: Conv1d(1, out_channels, 1, bias=True)
        ...
        self.temporaryDownsampler = AvgPool1d(2)
...
class ProGAN:
    ...
    def __progressive_downsampling(self, real_batch, depth, alpha):
        ...
        ds_real_samples = AvgPool1d(down_sample_factor)(real_batch)

        if depth > 0:
            prior_ds_real_samples = interpolate(AvgPool1d(prior_downsample_factor)(real_batch),
                                                scale_factor=2)
        else:
    ...
    def train(self, dataset, epochs, batch_sizes,
        ...
        for current_depth in range(start_depth, self.depth):
            ...
            print("Current resolution: %d" % current_res)
